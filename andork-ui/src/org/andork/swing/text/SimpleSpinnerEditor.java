...
/**
 * A simple base class for more specialized editors that displays a read-only
 * view of the model's current value with a <code>JTextField<code>.  Subclasses
 * can configure the <code>JTextField<code> to create
 * an editor that's appropriate for the type of model they
 * support and they may want to override
 * the <code>stateChanged</code> and <code>propertyChanged</code> methods, which
 * keep the model and the text field in sync.
 * <p>
 * This class defines a <code>dismiss</code> method that removes the editors
 * <code>ChangeListener</code> from the <code>JSpinner</code> that it's part of.
 * The <code>setEditor</code> method knows about
 * <code>DefaultEditor.dismiss</code>, so if the developer replaces an editor
 * that's derived from <code>JSpinner.DefaultEditor</code> its
 * <code>ChangeListener</code> connection back to the <code>JSpinner</code> will
 * be removed. However after that, it's up to the developer to manage their
 * editor listeners. Similarly, if a subclass overrides
 * <code>createEditor</code>, it's up to the subclasser to deal with their
 * editor subsequently being replaced (with <code>setEditor</code>). We expect
 * that in most cases, and in editor installed with <code>setEditor</code> or
 * created by a <code>createEditor</code> override, will not be replaced anyway.
 * <p>
 * This class is the <code>LayoutManager<code> for it's single
 * <code>JTextField</code> child. By default the child is just centered with the
 * parents insets.
 */
public class SimpleSpinnerEditor extends JPanel
	 ...
	 *
	 */
	private static final long serialVersionUID = 5806587753282136554L;
	 ...
	 * This <code>DefaultEditor</code> is it's own layout manager and it is
	 * added to the spinner's <code>ChangeListener</code> list. The constructor
	 * creates a single <code>JTextField<code> child,
	...
	/**
	 * This <code>LayoutManager</code> method does nothing. We're only managing
	 * a single child and there's no support for layout constraints.
	 *
	 * @param name
	 *            ignored
	 * @param child
	 ...
	 * spinners <code>ChangeListener</code> list.
	 *
	 * @param spinner
	 *            the <code>JSpinner</code> to disconnect this editor from; the
	 *            same spinner as was passed to the constructor.
	 */
	 ...
	 * subclasses of <codeJSpinner</code> may override the the
	...
	/**
	 * Returns the <code>JTextField</code> child of this editor. By default the
	 * text field is the first and only child of editor.
	 *
	 * @return the <code>JTextField</code> that gives the user access to the
	...
	/**
	 * Called by the <code>JTextField</code> <code>PropertyChangeListener</code>
	 * . When the <code>"value"</code> property changes, which implies that the
	 * user has typed a new number, we set the value of the spinners model.
	 * <p>
	 * This class ignores <code>PropertyChangeEvents</code> whose source is not
	 * the <code>JTextField</code>, so subclasses may safely make
	 * <code>this</code> <code>DefaultEditor</code> a
	 * <code>PropertyChangeListener</code> on other objects.
	 *
	 * @param e
	 *            the <code>PropertyChangeEvent</code> whose source is the
	 *            <code>JTextField</code> created by this class.
	 * @see #getTextField
	 */
	@Override
	 ...
	 * the <code>value</code> of the text field to the current value of the
	 * spinners model.
	 *
	 * @param e
	 *            not used
