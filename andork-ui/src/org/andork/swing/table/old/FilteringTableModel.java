...
/**
 * Provides an automatically-updated filtered view of a backing table model. The filter can be any implementation of the {@link Filter} interface.<br>
 * <br>
 * 
 * If more than a set number of rows have to be filtered at once, the filtering will be performed on a background thread to keep from tying up the Swing thread.
 * In this case, the filtered data will be temporarily out-of-date, but {@link #isRebuildingInBackground()} will return {@code true}.<br>
 * <br>
 * 
 * You can make the table display a wait cursor when background filtering is occuring using a {@link TableModelListener}:
 * 
 * <pre>
...
{
	/**
	 * Determines which rows a {@link FilteringTableModel} will include from its backing {@link TableModel}.
	 * 
	 * @author andy.edwards
	 */
	public static interface Filter
	{
		 ...
		 * @return if the row should be included, any non-null {@code Object}. This object may represent info about the filter result such as where exactly a
		 *         regular expression matched. If the row should not be included, the implementation should return {@code null}.
		 */
		public Object include( Object[ ] row );
	}
	
	/**
	 * A filter that includes all rows whose {@code value.toString()} in a specified {@code column} (or any column) contains a match for a specified
	...
	{
		/**
		 * Creates a filter that includes all rows whose {@code value.toString()} in the specified {@code column} contains a match for the specified
		 * {@code pattern} (the pattern does not have to match the entire value).
		 * 
		 * @param pattern
		 *            the filter pattern.
		...
		/**
		 * Creates a filter that includes all rows whose {@code value.toString()} in any column contains a match for the specified {@code pattern} (the pattern
		 * does not have to match the entire value).
		 * 
		 * @param pattern
		 *            the filter pattern.
		 * @param column
		...
		public boolean equals( Object o )
		{
			...
			if( o instanceof PatternFilter )
			{
				...
				return Java7.Objects.equals( column , f.column )
	...
	/**
	 * A filter that applies a delegate filter to determine which rows to include, and marks whether included rows passed one of a list of highlighting filters.
	 * This is so that a table can highlight rows that passed a given filter. It will indicate if a row passed any highlighting filter via a
	 * {@link HighlightingFilterResult} for that row.
	 * 
	 * @author james.a.edwards
	 */
	...
	/**
	 * Creates a filter that only includes rows that match each filter in an array.
	 * 
	 * @author james.a.edwards
	 */
	...
	{
		/**
		 * Creates a filter that only includes rows that match each filter in {@code filters}.
		 * 
		 * @param filters
		 *            the filters to use.
		 */
	...
	/**
	 * an ArrayList of Integers representing the indices the included rows in the backing model.
	 */
	...
	{
		if( !Java7.Objects.equals( this.filter , filter ) )
		{
			this.filter = filter;
			rebuildLater( );
		}
	...
	/**
	 * Makes {@code FilteringTableModel} start rebuilding the filtered results on a background thread. If a rebuild is currently in progress, it will be aborted
	 * and rebuilding will start over. (The rebuilt results will still be inserted into the table on the EDT.) This method returns immediately; it does not
	 * block until the rebuilding is complete.
	 * 
	...
	/**
	 * Makes {@code FilteringTableModel} rebuild the filtered results immediately on the calling thread (which must be the EDT). This method does not return
	...
	/**
	 * Queues the given {@link Runnable} to be invoked later when the background thread has finished rebuilding the filtered results. If the filtered results
	 * are up-to-date and no background thread is running, the {@code Runnable} will be invoked immediately on the EDT, and this method will not return until it
	 * returns.
	 * 
	 * @param r
	...
	private class PostRebuild implements Runnable
	{
		...
		public void run( )
		{
			...
			boolean structureChanged = columnNames == null;
