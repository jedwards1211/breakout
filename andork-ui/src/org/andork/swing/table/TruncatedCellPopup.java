...
/**
 * A popup that appears when the user moves the mouse over a table cell whose
 * contents are truncated. The popup uses the {@link TableCellRenderer} to
 * display the full contents of the cell.
 *
...
public class TruncatedCellPopup extends JPanel {
	private class RendererPanel extends JPanel {
		 ...
		 *
		 */
		private static final long serialVersionUID = 4327984162213032275L;
	 ...
	 *
	 */
	private static final long serialVersionUID = -5059622598127573017L;
	...
	private MouseEvent retargetEvent(MouseEvent e, Component target) {
		...
		return new MouseEvent(target, e.getID(), e.getWhen(), e.getModifiers(), newPoint.x, newPoint.y,
				e.getClickCount(), e.isPopupTrigger(), e.getButton());
	}

	private MouseEvent retargetEvent(MouseWheelEvent e, Component target) {
		Point newPoint = SwingUtilities.convertPoint(e.getComponent(), e.getPoint(), target);
		return new MouseWheelEvent(target, e.getID(), e.getWhen(), e.getModifiers(), newPoint.x, newPoint.y,
