 ...
 * <ul>
 * <li>Sorting is done on a background thread; as such it hangs the UI much less than {@link DefaultRowSorter} on large
 * tables
 * <li>If any changes occur while sorting, the background sort will restart efficiently
 * <li>Rows can be annotated by a {@link RowAnnotator} (this can be used, for example, to highlight rows matching a
 * filter)
 * </ul>
 * 
 * @param <M>
...
public abstract class AnnotatingRowSorter<M, I> extends RowSorter<M>
{
	...
	private Consumer<Runnable>					sortRunner;
	 ...
	 */
	public AnnotatingRowSorter( JTable table , Consumer<Runnable> sortRunner )
	{
	...
	/**
	 * Sets the <code>TableModel</code> to use as the underlying model for this <code>AnnotatingTableRowSorter</code>. A
	 * value of <code>null</code> can be used
	...
	/**
	 * Sets whether or not the specified column is sortable. The specified value is only checked when
	 * <code>toggleSortOrder</code> is invoked. It is still
	 * possible to sort on a column that has been marked as unsortable by directly setting the sort keys. The default is
	 * true.
	 * 
	 * @param column
	...
	/**
	 * Sets the sort keys. This creates a copy of the supplied {@code List}; subsequent changes to the supplied
	 * {@code List} do not effect this {@code DefaultRowSorter}. If the sort keys have changed this triggers a sort.
	 * 
	 * @param sortKeys
	 *            the new <code>SortKeys</code>; <code>null</code> is a shorthand for specifying an empty list,
	 *            indicating that the view should be unsorted
	 * @throws IllegalArgumentException
	 *             if any of the values in <code>sortKeys</code> are null or have a column index outside the range of
	 *             the model
	 */
	...
	/**
	 * Returns the current sort keys. This returns an unmodifiable {@code non-null List}. If you need to change the sort
	 * keys, make a copy of the returned {@code List}, mutate the copy and invoke {@code setSortKeys} with the new list.
	 * 
	...
	/**
	 * Sets the maximum number of sort keys. The number of sort keys determines how equal values are resolved when
	 * sorting. For example, assume a table row
	 * sorter is created and <code>setMaxSortKeys(2)</code> is invoked on it. The user clicks the header for column 1,
	 * causing the table rows to be sorted based
	 * on the items in column 1. Next, the user clicks the header for column 2, causing the table to be sorted based on
	 * the items in column 2; if any items in
	 * column 2 are equal, then those particular rows are ordered based on the items in column 1. In this case, we say
	 * that the rows are primarily sorted on
	 * column 2, and secondarily on column 1. If the user then clicks the header for column 3, then the items are
	 * primarily sorted on column 3 and secondarily
	 * sorted on column 2. Because the maximum number of sort keys has been set to 2 with <code>setMaxSortKeys</code>,
	 * column 1 no longer has an effect on the
	 * order.
	 * <p>
	 * The maximum number of sort keys is enforced by <code>toggleSortOrder</code>. You can specify more sort keys by
	 * invoking <code>setSortKeys</code> directly and they will all be honored. However if <code>toggleSortOrder</code>
	...
	/**
	 * If true, specifies that a sort should happen when the underlying model is updated (<code>rowsUpdated</code> is
	 * invoked). For example, if this is true and
	...
	/**
	 * Sets the annotator that annotates rows in the view. The annotator is applied before sorting. A value of
	 * <code>null</code> indicates that no rows should
	 * be annotated.
	 * <p>
	 * <code>RowAnnotator</code>'s <code>annotate</code> method is passed an <code>Entry</code> that wraps the
	 * underlying model. The number of columns in the <code>Entry</code> corresponds to the number of columns in the
	 * <code>ModelWrapper</code>. The identifier comes from the <code>ModelWrapper</code> as well.
	 * <p>
	...
	/**
	 * Sets the filter that determines which rows, if any, should be hidden from the view. The filter is applied before
	 * sorting. A value of <code>null</code> indicates all values from the model should be included.
	 * <p>
	 * <code>RowFilter</code>'s <code>include</code> method is passed an <code>Entry</code> that wraps the underlying
	 * model. The number of columns in the <code>Entry</code> corresponds to the number of columns in the
	 * <code>ModelWrapper</code>. The identifier comes from the <code>ModelWrapper</code> as well.
	 * <p>
	 * This method triggers a sort.
	 * 
	...
	/**
	 * Reverses the sort order from ascending to descending (or descending to ascending) if the specified column is
	 * already the primary sorted column;
	 * otherwise, makes the specified column the primary sorted column, with an ascending sort order. If the specified
	 * column is not sortable, this method has
	 * no effect.
	 * 
	 * @param column
	 ...
	 * <code>ModelWrapper.getStringValueAt</code> will be
	 * used, otherwise <code>ModelWrapper.getValueAt</code> will be used. It is up to subclasses, such as
	 * <code>AnnotatingTableRowSorter</code>, to honor this
	...
	/**
	 * Refreshes the modelToView mapping from that of viewToModel. If <code>unsetFirst</code> is true, all indices in
	 * modelToView are first set to -1.
	 */
	...
	/**
	 * Sets the <code>Comparator</code> to use when sorting the specified column. This does not trigger a sort. If you
	 * want to sort after setting the comparator
	 * you need to explicitly invoke <code>sort</code>.
	 * 
	 * @param column
	 *            the index of the column the <code>Comparator</code> is to be used for, in terms of the underlying
	 *            model
	 * @param comparator
	...
	/**
	 * Returns the <code>Comparator</code> for the specified column. This will return <code>null</code> if a
	 * <code>Comparator</code> has not been specified for
	 * the column.
	 * 
	 * @param column
	...
	/**
	 * Returns true if we should try and optimize the processing of the <code>TableModelEvent</code>. If this returns
	 * false, assume the event was dealt with and
	...
	/**
	 * <code>DefaultRowSorter.ModelWrapper</code> is responsible for providing the data that gets sorted by
	 * <code>DefaultRowSorter</code>. You normally do not
	 * interact directly with <code>ModelWrapper</code>. Subclasses of <code>DefaultRowSorter</code> provide an
	 * implementation of <code>ModelWrapper</code> wrapping another model. For example,
	 * <code>AnnotatingTableRowSorter</code> provides a <code>ModelWrapper</code> that wraps a <code>TableModel</code>.
	 * <p>
	 * <code>ModelWrapper</code> makes a distinction between values as <code>Object</code>s and <code>String</code>s.
	 * This allows implementations to provide a custom string converter to be used instead of invoking
	...
	protected abstract static class ModelWrapper<M, I>
	{
		...
		/**
		 * Returns the value as a <code>String</code> at the specified index. This implementation uses
		 * <code>toString</code> on the result from <code>getValueAt</code> (making sure to return an empty string for
		 * null values). Subclasses that override this method should never return null.
		 * 
		 * @param row
		 *            the row index
		 * @param column
		...
		/**
		 * Returns the identifier for the specified row. The return value of this is used as the identifier for the
		 * <code>RowFilter.Entry</code> that is passed
	 ...
	 * instance of this that is passed to the Filter.
	...
	/**
	 * Row is used to handle the actual sorting by way of Comparable. It will use the sortKeys to do the actual
	 * comparison.
	 */
	...
	public static class ExecutorServiceSortRunner implements Consumer<Runnable>
	{
	...
	private static class BackgroundSortTask<M, I> implements Runnable
	{
		...
		/**
		 * Indicates the contents are sorted. This is used if getSortsOnUpdates is false and an update event is
		 * received.
		 */
		private boolean								sorted;
		...
		private Comparator<Row>						rowComparator	= new Comparator<AnnotatingRowSorter.Row>( )
																	{
																		...
																		{
																			return BackgroundSortTask.this.compare(
																				o1.modelIndex , o2.modelIndex );
																		}
																	};
		...
		/**
		 * Sorts and filters the rows in the view based on the sort keys of the columns currently being sorted and the
		 * filter, if any, associated with this
		...
		/**
		 * Returns whether or not to convert the value to a string before doing comparisons when sorting. If true
		 * <code>ModelWrapper.getStringValueAt</code> will be used, otherwise <code>ModelWrapper.getValueAt</code> will
		 * be used. It is up to subclasses, such as <code>AnnotatingTableRowSorter</code>, to
		...
		/**
		 * Refreshes the modelToView mapping from that of viewToModel. If <code>unsetFirst</code> is true, all indices
		 * in modelToView are first set to -1.
		 */
		private void setModelToViewFromViewToModel( boolean unsetFirst )
		{
			int i;
		...
		/**
		 * Returns the <code>Comparator</code> for the specified column. This will return <code>null</code> if a
		 * <code>Comparator</code> has not been specified
		 * for the column.
		 * 
		 * @param column
		 *            the column to fetch the <code>Comparator</code> for, in terms of the underlying model
		...
		private class PreSort implements ExceptionRunnable
		{
			...
			public void run( )
			{
				...
				comparators = sorter.comparators == null ? null : Arrays.copyOf( sorter.comparators ,
					sorter.comparators.length );
				cachedSortKeys = sorter.cachedSortKeys == null ? null : Arrays.copyOf( sorter.cachedSortKeys ,
					sorter.cachedSortKeys.length );
				annotator = sorter.annotator;
				filter = sorter.filter;
				sortKeys = sorter.sortKeys;
				sorted = sorter.sorted;
				useToString = sorter.useToString == null ? null : Arrays.copyOf( sorter.useToString ,
					sorter.useToString.length );
