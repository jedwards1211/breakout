...
public class FalseDepthTest5b extends TorquescapeSetup
{
	...
	protected TorquescapeScene createScene( )
	{
		...
		obj.add( new Uniform1fv( ).name( "u_layer_depths" ).count( numLayers ).value( sequence( 0 , 0.2f , numLayers ) ) );
		obj.add( new Uniform4fv( ).name( "u_basecolor" ).value( 0 , 0 , 1.5f , 1 ) );
		obj.add( new Uniform4fv( ).name( "u_layer_colors" ).count( numLayers ).value(
				...
				0 , 0 , 0 , 0.2f ,
				0 , 0 , 0 , 0.1f ) );
		obj.add( new Uniform1fv( ).name( "u_u_period" ).value( 0.5f ) );
		obj.add( new Uniform1fv( ).name( "u_v_period" ).value( 2 * ( float ) Math.PI ) );
		obj.transpose( false );
