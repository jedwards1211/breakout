...
public class TorquescapeTest3 extends TorquescapeSetup
{
	public static void main( String[ ] args )
	{
		TorquescapeTest3 test = new TorquescapeTest3( );
	...
	protected TorquescapeScene createScene( )
	{
		...
		obj.fragmentShaderCode( new MatrixFragmentShader2( ).numLayers( 2 ).toString( ) );
		...
		obj.add( new Uniform1fv( ).name( "u_layer_depths" ).count( 2 ).value( 0 , 10f ) );
		obj.add( new Uniform4fv( ).name( "u_layer_colors" ).count( 2 ).value( 1 , 0 , 0 , 1 , 0 , 0 , 1f , 1 ) );
		obj.add( new Uniform1fv( ).name( "u_u_period" ).value( 10 ) );
		obj.add( new Uniform1fv( ).name( "u_v_period" ).value( 2 ) );
		obj.transpose( false );
	...
	public static Zone createZone( final Track track , List<Float> params )
	{
		IVertexAttrFn attrFn1 = new IVertexAttrFn( )
		{
			...
			{
				visitor.visit( param );
				visitor.visit( ( index / 2 ) * ( float ) Math.PI );
			}
		};
	...
	public static class MatrixFragmentShader2
	{
		...
		public String toString( )
		{
			...
			sb.append( "void main() {" );
			sb.append( "  for (int i = 0; i < " + numLayers + "; i++) {" );
			sb.append( "    float virtpos = v_vmpos * u_layer_depths[i] / dot(v_vmpos, -v_norm);" );
