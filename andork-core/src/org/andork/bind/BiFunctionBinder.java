...
public class BiFunctionBinder<T, U, R> extends Binder<R> {
	private Binder<T> upstreamT;
	private Binder<? extends U> upstreamU;
	private BiFunction<T, U, R> fn;
	private BiFunction<R, U, T> inverseFn;

	private R value;

	public BiFunctionBinder(BiFunction<T, U, R> fn, BiFunction<R, U, T> inverseFn) {
		this.fn = fn;
		this.inverseFn = inverseFn;
	}

	public BiFunctionBinder<T, U, R> bind(Binder<T> upstreamT, Binder<? extends U> upstreamU) {
		boolean needsUpdate = this.upstreamT != upstreamT || this.upstreamU != upstreamU;
	...
	public void set(R newValue) {
		...
		this.value = newValue;
		this.upstreamT.set(inverseFn.apply(newValue, upstreamU.get()));
	}
