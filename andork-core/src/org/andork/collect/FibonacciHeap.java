...
{
	public FibonacciHeap( K minPriority )
	{
		super( );
	...
	/*
	 * In order for all of the Fibonacci heap operations to complete in O(1), clients need to have O(1) access to any element in the heap. We make this work by
	 * having each insertion operation produce a handle to the node in the tree. In actuality, this handle is the node itself, but we guard against external
	 * modification by marking the internal fields private.
	 */
	public static final class Entry<K extends Comparable<K>, T>
	{
		private int			mDegree		= 0;		// Number of children
		private boolean		mIsMarked	= false;	// Whether this node is marked
													
		private Entry<K, T>	mNext;					// Next and previous elements in the list
		private Entry<K, T>	mPrev;
		
		private Entry<K, T>	mParent;				// Parent in the tree, if any.
													
		private Entry<K, T>	mChild;				// Child node, if any.
													
		private T			mElem;					// Element being stored here
		private K			mPriority;				// Its priority
		 ...
		 */
		private Entry( T elem , K priority )
		{
			mNext = mPrev = this;
			mElem = elem;
			mPriority = priority;
	...
	private final K		minPriority;
	
	/* Pointer to the minimum element in the heap. */
	private Entry<K, T>	mMin	= null;
	
	/* Cached size of the heap, so we don't have to recompute this explicitly. */
	private int			mSize	= 0;
	
	/**
	 * Inserts the specified element into the Fibonacci heap with the specified priority. Its priority must be a valid K, so you cannot set the priority to NaN.
	 * 
	 * @param value
	 *            The value to insert.
	 * @param priority
	 ...
	 */
	public Entry<K, T> enqueue( T value , K priority )
	{
		 ...
		 */
		Entry<K, T> result = new Entry<K, T>( value , priority );
	...
	/**
	 * Given two Fibonacci heaps, returns a new Fibonacci heap that contains all of the elements of the two heaps. Each of the input heaps is destructively
	 * modified by having all its elements removed. You can continue to use those heaps, but be aware that they will be empty after this call completes.
	 * 
	 * @param one
	 ...
	 */
	public static <K extends Comparable<K>, T> FibonacciHeap<K, T> merge( FibonacciHeap<K, T> one , FibonacciHeap<K, T> two )
	{
		/* Create a new FibonacciHeap to hold the result. */
		FibonacciHeap<K, T> result = new FibonacciHeap<K, T>( one.minPriority );
		
		/*
		 * Merge the two Fibonacci heap root lists together. This helper function also computes the min of the two lists, so we can store the result in the mMin
		 * field of the new heap.
		 */
	...
	public Entry<K, T> dequeueMin( )
	{
		...
		Entry<K, T> minElem = mMin;
		
		/*
		 * Now, we need to get rid of this element from the list of roots. There are two cases to consider. First, if this is the only element in the list of
		 * roots, we set the list of roots to be null by clearing mMin. Otherwise, if it's not null, then we write the elements next to the min element around
		 * the min element to remove it, then arbitrarily reassign the min.
		 */
		if( mMin.mNext == mMin )
		{ // Case one
			mMin = null;
		...
		/*
		 * Next, clear the parent fields of all of the min element's children, since they're about to become roots. Because the elements are stored in a
		 * circular list, the traversal is a bit complex.
		 */
		if( minElem.mChild != null )
		{
			/* Keep track of the first visited node. */
			Entry<K, ?> curr = minElem.mChild;
			do
			{
				curr.mParent = null;
		...
		/*
		 * Next, splice the children of the root node into the topmost list, then set mMin to point somewhere in that list.
		 */
		...
		/*
		 * Next, we need to coalsce all of the roots so that there is only one tree of each degree. To track trees of each size, we allocate an ArrayList where
		 * the entry at position i is either null or the unique tree of degree i.
		 */
		List<Entry<K, T>> treeTable = new ArrayList<Entry<K, T>>( );
		
		/*
		 * We need to traverse the entire list, but since we're going to be messing around with it we have to be careful not to break our traversal order
		 * mid-stream. One major challenge is how to detect whether we're visiting the same node twice. To do this, we'll spent a bit of overhead adding all of
		 * the nodes to a list, and then will visit each element of this list in order.
		 */
		List<Entry<K, T>> toVisit = new ArrayList<Entry<K, T>>( );
		
		/*
		 * To add everything, we'll iterate across the elements until we find the first element twice. We check this by looping while the list is empty or while
		 * the current element isn't the first element of that list.
		 */
		for( Entry<K, T> curr = mMin ; toVisit.isEmpty( ) || toVisit.get( 0 ) != curr ; curr = curr.mNext )
			toVisit.add( curr );
		
		/* Traverse this list and perform the appropriate unioning steps. */
		for( Entry<K, T> curr : toVisit )
		{
			...
			while( true )
			{
				...
				Entry<K, T> other = treeTable.get( curr.mDegree );
				 ...
				 */
				Entry<K, T> min = ( other.mPriority.compareTo( curr.mPriority ) < 0 ) ? other : curr;
				Entry<K, T> max = ( other.mPriority.compareTo( curr.mPriority ) < 0 ) ? curr : other;
			...
			/*
			 * Update the global min based on this node. Note that we compare for <= instead of < here. That's because if we just did a reparent operation that
			 * merged two different trees of equal priority, we need to make sure that the min pointer points to the root-level one.
			 */
			if( curr.mPriority.compareTo( mMin.mPriority ) <= 0 )
				mMin = curr;
		}
		return minElem;
	}
	...
	/**
	 * Decreases the key of the specified element to the new priority. If the new priority is greater than the old priority, this function throws an
	 * IllegalArgumentException. The new priority must be a finite K, so you cannot set the priority to be NaN, or +/- infinity. Doing so also throws an
	 ...
	 *             If the new priority exceeds the old priority, or if the argument is not a finite K.
	 */
	public void decreaseKey( Entry<K, T> entry , K newPriority )
	{
		checkPriority( newPriority );
		if( newPriority.compareTo( entry.mPriority ) > 0 )
	...
	{
		/*
		 * Use decreaseKey to drop the entry's key to -infinity. This will guarantee that the node is cut and set to the global minimum.
		 */
		decreaseKeyUnchecked( entry , minPriority );
	...
	/**
	 * Utility function which, given a user-specified priority, checks whether it's a valid K and throws an IllegalArgumentException otherwise.
	 * 
	 * @param priority
	 ...
	 *             If it is not valid.
	 */
	protected void checkPriority( K priority )
	{
	}
	
	/**
	 * Utility function which, given two pointers into disjoint circularly- linked lists, merges the two lists together into one circularly-linked list in O(1)
	 * time. Because the lists may be empty, the return value is the only pointer that's guaranteed to be to an element of the resulting list.
	 * 
	 * This function assumes that one and two are the minimum elements of the lists they are in, and returns a pointer to whichever is smaller. If this
	 * condition does not hold, the return value is some arbitrary pointer into the doubly-linked list.
	 * 
	 * @param one
	 ...
	 */
	private static <K extends Comparable<K>, T> Entry<K, T> mergeLists( Entry<K, T> one , Entry<K, T> two )
	{
		/*
		 * There are four cases depending on whether the lists are null or not. We consider each separately.
		 */
		if( one == null && two == null )
		{ // Both null, resulting list is null.
			return null;
		}
		else if( one != null && two == null )
		{ // Two is null, result is one.
			return one;
		}
		else if( one == null && two != null )
		{ // One is null, result is two.
			return two;
		}
		else
		{ // Both non-null; actually do the splice.
			 ...
			 * |--N->|two | | | | | | |<-P--| | | | | P +----+ +----+ +----+ | | ^ | | | | +-------------+ | +-----------------+
			 */
			Entry<K, T> oneNext = one.mNext; // Cache this since we're about to overwrite it.
			one.mNext = two.mNext;
			one.mNext.mPrev = one;
			two.mNext = oneNext;
			two.mNext.mPrev = two;
			...
			return one.mPriority.compareTo( two.mPriority ) < 0 ? one : two;
		}
	}
	 ...
	 */
	private void decreaseKeyUnchecked( Entry<K, T> entry , K priority )
	{
		/* First, change the node's priority. */
		entry.mPriority = priority;
		
		/*
		 * If the node no longer has a higher priority than its parent, cut it. Note that this also means that if we try to run a delete operation that
		 * decreases the key to -infinity, it's guaranteed to cut the node from its parent.
		 */
		if( entry.mParent != null && entry.mPriority.compareTo( entry.mParent.mPriority ) <= 0 )
			cutNode( entry );
		
		/*
		 * If our new value is the new min, mark it as such. Note that if we ended up decreasing the key in a way that ties the current minimum priority, this
		 * will change the min accordingly.
		 */
		if( entry.mPriority.compareTo( mMin.mPriority ) <= 0 )
			mMin = entry;
	}
	 ...
	 */
	private void cutNode( Entry<K, T> entry )
	{
		...
		/*
		 * If the node is the one identified by its parent as its child, we need to rewrite that pointer to point to some arbitrary other child.
		 */
