...
public final class FibonacciHeap<K extends Comparable<K>, T> {
	 ...
	 */
	public static <K extends Comparable<K>, T> FibonacciHeap<K, T> merge(FibonacciHeap<K, T> one,
			FibonacciHeap<K, T> two) {
	...
	private static <K extends Comparable<K>, T> Entry<K, T> mergeLists(Entry<K, T> one, Entry<K, T> two) {
		...
		if (one == null && two == null) { // Both null, resulting list is null.
			 ...
			 * +----+ +----+ | | ^ | | | | +-------------+ | +-----------------+
			 */
			Entry<K, T> oneNext = one.mNext; // Cache this since we're about to
												// overwrite it.
			one.mNext = two.mNext;
			one.mNext.mPrev = one;
			two.mNext = oneNext;
	 ...
	 * it's a valid K and throws an IllegalArgumentException otherwise.
	 *
	 * @param priority
	 ...
	 * IllegalArgumentException. The new priority must be a finite K, so you
	 * cannot set the priority to be NaN, or +/- infinity. Doing so also throws
	 ...
	 * priority. Its priority must be a valid K, so you cannot set the priority
	 * to NaN.
	 *
	 * @param value
	 *            The value to insert.
