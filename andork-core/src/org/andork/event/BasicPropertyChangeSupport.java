...
/**
 * This class is an adaptation of {@link PropertyChangeSupport} for {@link BasicPropertyChangeListener}.
 */
public class BasicPropertyChangeSupport implements Serializable
{
	...
	/**
	 * The listener list. A copy-on-write array is used instead of an List because the latter would run the risk of {@link ConcurrentModificationException}s (if
	...
	/**
	 * Add a BasicPropertyChangeListener to the listener list. The listener is registered for all properties. The same listener object may be added more than
	 * once, and will be called as many times as it is added. If <code>listener</code> is null, no exception is thrown and no action is taken.
	 * 
	 * @param listener
	...
	/**
	 * Remove a BasicPropertyChangeListener from the listener list. This removes a BasicPropertyChangeListener that was registered for all properties. If
	 * <code>listener</code> was added more than once to the same event source, it will be notified one less time after being removed. If <code>listener</code>
	...
	/**
	 * Returns an array of all the listeners that were added to the BasicPropertyChangeSupport object with addPropertyChangeListener().
	 * <p>
	 ...
	 * <code>BasicPropertyChangeListenerProxy</code>s. If the calling method is interested in distinguishing the listeners then it must test each element to see
	 * if it's a <code>BasicPropertyChangeListenerProxy</code>, perform the cast, and examine the parameter.
	 * 
	 * <pre>
	...
	/**
	 * Add a BasicPropertyChangeListener for a specific property. The listener will be invoked only when a call on firePropertyChange names that specific
	 * property. The same listener object may be added more than once. For each property, the listener will be invoked the number of times it was added for that
	 * property. If <code>propertyName</code> or <code>listener</code> is null, no exception is thrown and no action is taken.
	 * 
	 * @param propertyName
	...
	/**
	 * Remove a BasicPropertyChangeListener for a specific property. If <code>listener</code> was added more than once to the same event source for the
	 * specified property, it will be notified one less time after being removed. If <code>propertyName</code> is null, no exception is thrown and no action is
	 * taken. If <code>listener</code> is null, or was never added for the specified property, no exception is thrown and no action is taken.
	 * 
	 * @param propertyName
	...
	/**
	 * Report an <code>int</code> bound indexed property update to any registered listeners.
	 * <p>
	 * No event is fired if old and new values are equal and non-null.
	 * <p>
	...
	/**
	 * Report a <code>boolean</code> bound indexed property update to any registered listeners.
	 * <p>
	 * No event is fired if old and new values are equal and non-null.
	 * <p>
	 * This is merely a convenience wrapper around the more general fireIndexedPropertyChange method which takes Object values.
	...
	/**
	 * Check if there are any listeners for a specific property, including those registered on all properties. If <code>propertyName</code> is null, only check
	 * for listeners registered on all properties.
	 * 
	 * @param propertyName
	 *            the property name.
	...
	/**
	 * Hashtable for managing listeners for specific properties. Maps property names to BasicPropertyChangeSupport objects.
	 * 
	 * @serial
	 * @since 1.2
	 */
	...
	/**
	 * Contains delegates for the only methods of {@link BasicPropertyChangeSupport} that users of a class that incorporates a
	 * {@code BasicPropertyChangeSupport} should use.
	 * 
	 * @author Andy
	 */
	public class External
	{
		 ...
		 * <p>
		 * If some listeners have been added with a named property, then the returned array will be a mixture of PropertyChangeListeners and
		 * <code>BasicPropertyChangeListenerProxy</code>s. If the calling method is interested in distinguishing the listeners then it must test each element to
		 * see if it's a <code>BasicPropertyChangeListenerProxy</code>, perform the cast, and examine the parameter.
		 * 
		 * <pre>
		 * BasicPropertyChangeListener[ ] listeners = bean.getPropertyChangeListeners( );
		 * for( int i = 0 ; i &lt; listeners.length ; i++ )
		...
		/**
		 * Add a BasicPropertyChangeListener for a specific property. The listener will be invoked only when a call on firePropertyChange names that specific
		 * property. The same listener object may be added more than once. For each property, the listener will be invoked the number of times it was added for
		 * that property. If <code>propertyName</code> or <code>listener</code> is null, no exception is thrown and no action is taken.
		 * 
		 * @param propertyName
		 *            The name of the property to listen on.
		 * @param listener
		...
		/**
		 * Remove a BasicPropertyChangeListener for a specific property. If <code>listener</code> was added more than once to the same event source for the
		 * specified property, it will be notified one less time after being removed. If <code>propertyName</code> is null, no exception is thrown and no action
		 * is taken. If <code>listener</code> is null, or was never added for the specified property, no exception is thrown and no action is taken.
		 * 
		 * @param propertyName
		 *            The name of the property that was listened on.
		 * @param listener
		...
		/**
		 * Add a BasicPropertyChangeListener to the listener list. The listener is registered for all properties. The same listener object may be added more
		...
		/**
		 * Remove a BasicPropertyChangeListener from the listener list. This removes a BasicPropertyChangeListener that was registered for all properties. If
		 * <code>listener</code> was added more than once to the same event source, it will be notified one less time after being removed. If
