...
public class RfStarTreeTest {
	public static void main(String[] args) {
		...
		final JPanel drawPanel = new JPanel() {
			 ...
			 *
			 */
			private static final long serialVersionUID = 6229760833176896190L;
			...
			private void draw(Node<String> node, Graphics2D g) {
				...
				while (!queue.isEmpty()) {
					...
					for (Node<String> next : queue) {
						rect.setFrame(next.mbr[0], next.mbr[1], next.mbr[dimension] - next.mbr[0],
								next.mbr[dimension + 1] - next.mbr[1]);
						g.fill(rect);
						g.draw(rect);
		...
		MouseAdapter adapter = new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				Leaf<String> node = tree.createLeaf(mbr, Integer.toString(nodeCount++));
				nodes.add(node);
