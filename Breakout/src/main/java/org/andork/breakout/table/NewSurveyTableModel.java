...
public class NewSurveyTableModel extends NiceTableModel<QObject<NewSurveyTableModel.Row>>
{
		...
		STRING
		{
			Column<QObject<Row>> createColumn( String name )
			{
				return MapColumn.newInstance(
						QObjectColumn.newInstance( Row.spec , Row.customAttrs ) , name , String.class , ( ) -> QLinkedHashMap.newInstance( ) );
			}
		} ,
		INTEGER
		{
	...
	public static final class CustomColumn implements Column<QObject<Row>>
	{
		...
		SelfDescribingFormat<?>				defaultFormat;
		
		public CustomColumn( String name , CustomColumnType type , SelfDescribingFormat<?> defaultFormat )
		{
			super( );
	...
	public NewSurveyTableModel( )
	{
		setColumns( Arrays.asList(
				...
				shotMeasurementColumn = FormattedTextColumn.newInstance( QObjectColumn.newInstance( Row.spec , Row.shotMeasurement ) ,
						ShotMeasurement.class , ( ) -> new FormattedText( ) ) ,
				fromCrossSectionColumn = FormattedTextColumn.newInstance( QObjectColumn.newInstance( Row.spec , Row.fromCrossSection ) ,
						CrossSection.class , ( ) -> new FormattedText( ) ) ,
				toCrossSectionColumn = FormattedTextColumn.newInstance( QObjectColumn.newInstance( Row.spec , Row.toCrossSection ) ,
						CrossSection.class , ( ) -> new FormattedText( ) )
				) );
	...
	public void addCustomColumn( CustomColumn newColumn )
	{
		for( Column<?> column : getColumns( ) )
		{
			if( newColumn.getColumnName( ).equals( column.getColumnName( ) ) )
			{
				throw new IllegalArgumentException( "A column named \"" + newColumn.getColumnName( ) + "\" already exists" );
			}
		}
	...
	{
		return CollectionUtils.toArrayList( getColumns( ).stream( ).filter(
				c -> c instanceof CustomColumn ).<CustomColumn>map( c -> ( CustomColumn ) c ) );
	}
	...
	public static class NewSurveyTableModelCopier extends AbstractTableModelCopier<NewSurveyTableModel>
	{
		...
		public NewSurveyTableModel createEmptyCopy( NewSurveyTableModel model )
		{
			...
			for( CustomColumn column : model.getCustomColumns( ) )
			{
				copy.addCustomColumn( column );
			}
			return copy;
		}
