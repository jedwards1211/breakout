...
/**
 * This is the superclass for Paints which use a multiple color gradient to fill in their raster. It provides storage for variables and enumerated values common
 * to {@code LinearGradientPaint} and {@code RadialGradientPaint}.
 * 
...
public abstract class MultipleGradientPaint implements Paint
{
	...
	/**
	 * The following fields are used only by MultipleGradientPaintContext to cache certain values that remain constant and do not need to be recalculated for
	 * each context created from this paint instance.
	 */
	ColorModel					model;
	 ...
	 * @param colorSpace
	 *            which color space to use for interpolation, either {@code SRGB} or {@code LINEAR_RGB}
	 * @param gradientTransform
	 *            transform to apply to the gradient
	 * 
	 * @throws NullPointerException
	 *             if {@code fractions} array is null, or {@code colors} array is null, or {@code gradientTransform} is null, or {@code cycleMethod} is null, or
	 *             {@code colorSpace} is null
	 * @throws IllegalArgumentException
	 *             if {@code fractions.length != colors.length}, or {@code colors} is less than 2 in size, or a {@code fractions} value is less than 0.0 or
	 *             greater than 1.0, or the {@code fractions} are not provided in strictly increasing order
	 */
	...
	/**
	 * Returns a copy of the array of floats used by this gradient to calculate color distribution. The returned array always has 0 as its first value and 1 as
	 * its last value, with increasing values in between.
	 * 
	...
	/**
	 * Returns a copy of the array of colors used by this gradient. The first color maps to the first value in the fractions array, and the last color maps to
	 * the last value in the fractions array.
	 * 
	 ...
	 * 
	 * @return an integer value representing the transparency mode for this Paint object
