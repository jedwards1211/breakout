...
public class GeneralViewXformOrbitAnimation implements Animation
{
	...
	public GeneralViewXformOrbitAnimation setUp( float[ ] origin , float[ ] startViewXform , float[ ] endForward , float[ ] endRight )
	{
		invAffine( startViewXform , startXform );
		...
		scaleAdd3( upAmt , this.endUp , endLocation , endLocation );

		restOfSetUp( );
		return this;
	}

	public GeneralViewXformOrbitAnimation setUpWithEndLocation( float[ ] startViewXform , float[ ] endLocation , float[ ] endForward , float[ ] endRight )
	{
		invAffine( startViewXform , startXform );
	...
	{
		if( Math.abs( dot3( startRight , endRight ) - 1f ) < 1e-6f || Math.abs( dot3( startForward , endForward ) - 1f ) < 1e-6f )
		{
			linear = true;
			return;
		}
		...
		startPan = ( double ) Math.atan2( startHorizontal[ 2 ] , startHorizontal[ 0 ] );
		endPan = ( double ) Math.atan2( endHorizontal[ 2 ] , endHorizontal[ 0 ] );
		...
		if( llx.findIntersection( ) && Math.abs( totalPan ) > minRotation && Math.PI - Math.abs( totalPan ) > minRotation )
		{
			...
			startOrigin[ 2 ] = endOrigin[ 2 ] = llx.x[ 1 ]; // note that llx.x[1] is the z coordinate
															// llx.x[2] would be out of bounds
		}
		...
		startTilt = ( double ) Math.atan2( alignedStartForward[ 1 ] , alignedStartForward[ 0 ] );
		endTilt = ( double ) Math.atan2( alignedEndForward[ 1 ] , alignedEndForward[ 0 ] );
		...
		if( llx.findIntersection( ) && Math.abs( totalTilt ) > minRotation && Math.PI - Math.abs( totalTilt ) > minRotation )
		{
