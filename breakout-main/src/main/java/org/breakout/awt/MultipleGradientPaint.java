 ...
 * common to {@code LinearGradientPaint} and {@code RadialGradientPaint}.
 *
...
public abstract class MultipleGradientPaint implements Paint {
	...
	/**
	 * The following fields are used only by MultipleGradientPaintContext to
	 * cache certain values that remain constant and do not need to be
	 ...
	 * @param colorSpace
	 *            which color space to use for interpolation, either
	 ...
	 *             is null, or {@code gradientTransform} is null, or
	 *             {@code cycleMethod} is null, or {@code colorSpace} is null
	 * @throws IllegalArgumentException
	 *             if {@code fractions.length != colors.length}, or
	 *             {@code colors} is less than 2 in size, or a {@code fractions}
	 *             value is less than 0.0 or greater than 1.0, or the
	 *             {@code fractions} are not provided in strictly increasing
	 *             order
	 */
	...
	/**
	 * Returns a copy of the array of colors used by this gradient. The first
	 * color maps to the first value in the fractions array, and the last color
	 * maps to the last value in the fractions array.
	 *
	 ...
	 * color distribution. The returned array always has 0 as its first value
	 * and 1 as its last value, with increasing values in between.
	 *
	 ...
	 *
	 * @return an integer value representing the transparency mode for this
	 *         Paint object
