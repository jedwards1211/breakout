...
public class Proj4Utils {
	...
	public static ProjCoordinate convert(CoordinateTransform xform, ProjCoordinate coord) {
	...
	public static ProjCoordinate
		convert(ProjCoordinate coord, CoordinateReferenceSystem fromSys, CoordinateReferenceSystem toSys) {
		return convert(coord, fromSys, coord, toSys);
	}

	public static ProjCoordinate
		convertToGeographic(ProjCoordinate fromCoord, CoordinateReferenceSystem sys, ProjCoordinate toCoord) {
		return convert(fromCoord, sys, toCoord, sys.createGeographic());
	}

	public static ProjCoordinate convertToGeographic(ProjCoordinate coord, CoordinateReferenceSystem sys) {
		return convertToGeographic(coord, sys, coord);
	}

	public static ProjCoordinate convert(ProjCoordinate fromCoord, String fromSys, CoordinateReferenceSystem toSys) {
		return convert(fromCoord, factory.createFromParameters(null, fromSys), toSys);
	}

	public static ProjCoordinate
		convert(ProjCoordinate fromCoord, String fromSys, ProjCoordinate toCoord, String toSys) {
		return convert(
			fromCoord,
	...
	public static ProjCoordinate convert(ProjCoordinate coord, String fromSys, String toSys) {
		return convert(coord, fromSys, coord, toSys);
	}

	public static ProjCoordinate convertToGeographic(ProjCoordinate fromCoord, String sys, ProjCoordinate toCoord) {
		return convertToGeographic(fromCoord, factory.createFromParameters(null, sys), toCoord);
	}

	public static ProjCoordinate convertToGeographic(ProjCoordinate coord, String sys) {
		return convertToGeographic(coord, factory.createFromParameters(null, sys));
	}

	public static ProjCoordinate convertToGeocentric(ProjCoordinate coord, CoordinateReferenceSystem fromSys) {
	...
	public static ProjCoordinate convertToGeocentric(ProjCoordinate coord, String fromSys) {
	...
	public static int getUtmZone(UnitizedDouble<Angle> longitude, UnitizedDouble<Angle> latitude) {
		if (longitude.unit == Angle.degrees) {
			return getUtmZoneDegrees(longitude.doubleValue(Angle.degrees), latitude.doubleValue(Angle.degrees));
		}
		return getUtmZoneRadians(longitude.doubleValue(Angle.radians), latitude.doubleValue(Angle.radians));
	}
